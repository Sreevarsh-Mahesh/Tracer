...
import { getCustomFunnelMetrics, getTrackedElements } from "@/lib/tracer-store";
...
export function FunnelPanel({ refreshToken }: { refreshToken: number }) {
  ...
  const funnelMetrics = clientReady ? getCustomFunnelMetrics(steps) : [];
  ...
  function attachListener() {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      const label = trackedElement.dataset.tracerLabel ?? trackedElement.textContent?.trim() ?? elementId;

      setSteps((currentSteps) => {
        if (currentSteps.includes(elementId)) {
          setBuilderHint(`${label} is already part of this funnel. Choose the next unique stage.`);
          return currentSteps;
        }

        setBuilderHint(`${label} added as step ${currentSteps.length + 1}.`);
        return [...currentSteps, elementId];
      });
    };

    frameDocument.addEventListener("click", handleClick, true);
    paintTrackedSteps(frameDocument, steps);
    ...
    <Box sx={{ position: "relative", display: "flex", overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          {/* Step chips */}
          {steps.length > 0 && (
            <Box sx={{ px: 2.5, pt: 1.5, pb: 1, borderBottom: "1px solid rgba(255,255,255,0.04)", flexShrink: 0 }}>
              ...
              <Card sx={{ backgroundColor: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.08)" }}>
                <CardContent sx={{ p: 2.5 }}>
                  <Typography color="text.secondary" variant="body2" sx={{ lineHeight: 1.7 }}>
                    Click <strong>Home → Product → Add to Cart → Checkout</strong> in the iframe to build the custom journey.
                  </Typography>
                </CardContent>
              </Card>
            ) : (
