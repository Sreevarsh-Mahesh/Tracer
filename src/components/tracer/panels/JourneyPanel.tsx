...
import { ChevronLeft, ChevronRight, Layers, Pause, Play, RotateCcw, X } from "lucide-react";
import { getJourneyClusters } from "@/lib/tracer-store";
...
export function JourneyPanel({ refreshToken }: { refreshToken: number }) {
  const [clientReady, setClientReady] = useState(false);
  const clusters = clientReady ? getJourneyClusters() : [];
  ...
  useEffect(() => {
    if (!clusters.find((cluster) => cluster.id === selectedClusterId)) {
      setSelectedClusterId(clusters[0]?.id ?? null);
    }
  }, [clusters, selectedClusterId, refreshToken]);
  ...
  const replayDurationMs = !selectedCluster
    ? 1000
    : Math.max(1000, ...selectedCluster.streams.map((stream) => stream.points[stream.points.length - 1]?.ts ?? 0));

  useEffect(() => {
    if (!playing || !selectedCluster) {
      return;
    }

    const interval = window.setInterval(() => {
      setPlayheadMs((value) => (value + 180 >= replayDurationMs ? 0 : value + 180));
    }, 120);
              ...
              >
                <Box>
                  ...
                  <Typography color="text.secondary">
                    Multi-cursor overlay groups similar sessions into a single visual summary.
                  </Typography>
                </Box>
        ...
        >
            ...
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Box>
                ...
                <Typography variant="caption" color="text.secondary">
                  {clusters.length} behavior groups
                </Typography>
              </Box>
            </Stack>
            <IconButton
          ...
          <Box sx={{ px: 2.5, py: 1.5, borderBottom: "1px solid rgba(255,255,255,0.04)", flexShrink: 0 }}>
            <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.6 }}>
              Select a category to replay how that class of users navigates the app.
            </Typography>
          </Box>
                  ...
                  <Card
                    ...
                    sx={{
                      ...
                      "&:hover": {
                        ...
                        boxShadow: `0 4px 16px ${cluster.color}22`
                      }
                    }}
                  >
                    ...
                    <CardContent sx={{ p: 2, pl: isSelected ? 2.5 : 2 }}>
                      <Stack spacing={1}>
                        ...
                        <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap>
                          <Chip
                            label={`⏱ ${formatDuration(cluster.avgDurationMs)}`}
                            size="small"
                            sx={{ height: 18, fontSize: "0.68rem" }}
                          />
                          <Chip
                            label={`😤 ${cluster.avgFrustrationIndex}`}
                            size="small"
                            sx={{ height: 18, fontSize: "0.68rem" }}
                          />
                        </Stack>
          ...
          >
            <Typography variant="caption" color="text.secondary">
              Click a cluster to switch replay →
            </Typography>
          </Box>
        </Card>
      </Box>
